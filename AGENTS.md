...
当前优先建议处理 Issue #6：持久化监控项目路径。开始前先读取 Issue 最新内容，并从最新 `main` 创建独立短分支；如果 Issue 状态或优先级已经变化，以 GitHub 为准。
...
- `preload.js`：安全 IPC 桥接层，向渲染进程暴露受控的 `window.ccAPI`。
...
- `renderer/css/`：宠物、气泡、详情面板和后续设置面板的外观。
...
运行时用户数据放在 `data/` 下并保持 Git 忽略，除非用户明确要求提交。

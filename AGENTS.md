...
`AGENTS.md` 只负责长期协作规则、当前架构约束和最近的续接入口，不承担工作记录职责。

- 仅当协作流程、当前架构、关键安全边界、验证方式或续接入口发生实质变化时修改。
- 更新当前事实时直接替换失效描述，不在后面追加按时间排列的变更记录。
- 不记录提交号、PR 流水、逐 Issue 完成明细、一次性测试结果或可从 GitHub 与 Git 查询的历史。
- 修改前以当前代码和 GitHub Issue 为依据，区分“已经实现”“正在开发”和“计划进行”，不要把计划提前写成稳定基线。
- 与 `README.md`、Issue 或学习笔记重复的内容应缩短为必要约束和链接；定期删除失效、重复或不再影响决策的段落。
- 保持内容简洁。能用短句说明时不展开教程；需要详细原理时更新 `docs/notes/`，并从本文件按需链接。
...
- `app` 通过 `requestSingleInstanceLock()` 保证只有主实例拥有 `CCMonitor`、`Tray` 和窗口；第二次启动只唤起主实例。隐藏宠物或关闭详情不会停止监控；托盘“退出”或宠物窗口 Alt+F4 才退出应用并释放资源。
...
5. 合并前检查 `AGENTS.md` 是否仍准确；需要修改时遵循“如何维护本文件”，不追加逐任务日志。
...
开始任务时默认只读取对应 GitHub Issue 和 `README.md`。遇到相关主题时再选择下列资料，不要求一次性全部加载：

- 产品范围或整体架构：`docs/superpowers/specs/2026-07-01-desktop-pet-design.md`。
- Claude Code 数据桥接、监听或隐私：`docs/superpowers/specs/2026-07-10-claude-statusline-monitor-design.md`、`docs/notes/10-文件监听故障与不可信数据.md`。
- IPC、配置或外部字符串安全：`docs/notes/11-innerHTML与外部字符串.md`、`docs/notes/12-用户配置、安全IPC与派生状态.md`。
- 多窗口、置顶、任务栏或托盘：`docs/notes/13-多窗口职责与独立置顶.md`、`docs/notes/15-系统托盘与应用生命周期.md`。
- 测试、PR 或交付流程：`docs/notes/09-分层测试与GitHub验收.md`。
